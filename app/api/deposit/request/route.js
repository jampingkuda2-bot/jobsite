...
export async function POST(req) {
  try {
    ...
    if (!amt || amt <= 0) return Response.json({ error: "Jumlah tidak valid" }, { status: 400 });
    if (!paymentMethod) return Response.json({ error: "Metode pembayaran wajib" }, { status: 400 });
    if (!proofUrl) return Response.json({ error: "Bukti transfer wajib diunggah" }, { status: 400 });
